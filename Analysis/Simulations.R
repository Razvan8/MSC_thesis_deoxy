...
##On train
#fit=hierNet(X_only_main, y_all, lam=3, diagonal = FALSE, stand.main=FALSE,tol=1e-6)
#fit
...
myWeakHierNet<-WeakHierNetUnscaled (X=X_only_main, Beta_plus_init=matrix(0,nrow = dim(X_only_main)[2], ncol = 1), Beta_minus_init=matrix(0,nrow = dim(X_only_main)[2], ncol = 1), 
                                    Theta_init=matrix(0, ncol = dim(X_only_main)[2], nrow = dim(X_only_main)[2]), y=y_all, lambda=lmd, t=t, tol=1e-9, 
...
lasso_model <- glmnet(X, y_all, alpha = 1, lambda=0.0104)
